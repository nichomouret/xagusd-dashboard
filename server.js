...
// Symboles argent testés dans l'ordre
const SYMBOLS = ['XAG/USD', 'SILVER', 'XAGUSD'];
...
async function getPrice() {
  for (const sym of SYMBOLS) {
    try {
      ...
      if (p > 5 && p < 500) return { price: p, symbol: sym };
...
server.listen(PORT, () => console.log(`XAG/USD Proxy v2 running on port ${PORT}`));
